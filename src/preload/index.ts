...
const api = {
  ...
  musicCache: {
    ...
    setMaxSize: (bytes: number) => ipcRenderer.invoke('music-cache:set-max-size', bytes),
    getDetailedInfo: () => ipcRenderer.invoke('music-cache:get-detailed-info'),
    setAutoCleanup: (enabled: boolean) => ipcRenderer.invoke('music-cache:set-auto-cleanup', enabled),
    setCleanupThreshold: (threshold: number) => ipcRenderer.invoke('music-cache:set-cleanup-threshold', threshold),
    markDownloaded: (songKey: string) => ipcRenderer.invoke('music-cache:mark-downloaded', songKey),
    unmarkDownloaded: (songKey: string) => ipcRenderer.invoke('music-cache:unmark-downloaded', songKey),
    cleanupAuto: () => ipcRenderer.invoke('music-cache:cleanup-auto')
  },
  // 文件读取
  file: {
