...
const purgeExpiredMediaCaptureGrants = () => {
  const now = Date.now()
...
const getMediaCaptureGrant = (webContentsId: number) => {
  purgeExpiredMediaCaptureGrants()
  return mediaCaptureGrants.get(webContentsId) || null
}

const isMainWindowWebContents = (webContents: WebContents | null | undefined) => {
  return !!mainWindow && !!webContents && !mainWindow.isDestroyed() && mainWindow.webContents.id === webContents.id
}

const authorizeMediaCaptureForWebContents = (webContentsId: number) => {
...
const consumeScreenSourceLookupGrant = (webContentsId: number) => {
...
const hasMediaPermissionGrant = (webContentsId: number) => {
  const grant = getMediaCaptureGrant(webContentsId)
  return !!grant && grant.remainingMediaRequests > 0
}

const consumeMediaPermissionGrant = (webContentsId: number) => {
  const grant = getMediaCaptureGrant(webContentsId)
...
const sendPlaybackControl = (channel: 'music-control' | 'playPrev' | 'playNext') => {
  if (!mainWindow || mainWindow.isDestroyed() || mainWindow.webContents.isDestroyed()) return
  mainWindow.webContents.send(channel)
}

const isMacStatusBarLyricTrayInteractive = () =>
  ...
  if (g.__ceru_tray__) {
    ...
    tray =
      process.platform === 'darwin'
        ? new Tray(trayImage, MAC_TRAY_GUID)
        : new Tray(trayImage)
...
app.whenReady().then(async () => {
  ...
  // 初始化音乐缓存服务
  musicCacheService.initialize().catch(console.error)
  ...
  if (mainWindow) {
    ...
    const lyricState = configManager.get('lyric', { isOpen: false }) as { isOpen?: boolean }
    if (lyricState.isOpen) {
      lyricWindow.create()
    }
