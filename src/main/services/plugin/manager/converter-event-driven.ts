...
async function getPic(source, musicInfo) {
  ...
  console.log('[' + pluginInfo.name + '] 使用事件驱动方式获取 ' + source + ' 封面');

  try {
    // 调用插件的请求处理器，action 为 'getPic'
    ...
    // 检查结果是否有效
    if (!result) {
      const errorMessage = '获取 ' + source + ' 封面失败: 返回结果为空';
      console.error('[' + pluginInfo.name + '] Error: ' + errorMessage);
      throw new Error(errorMessage);
    }
    ...
    if (typeof result === 'object' && result.error) {
      const errorMessage = result.error || '获取 ' + source + ' 封面失败';
      console.error('[' + pluginInfo.name + '] Error: ' + errorMessage);
      throw new Error(errorMessage);
    }
    ...
    console.log('[' + pluginInfo.name + '] Got pic: ' + (typeof result === 'string' ? result : result.url || result));
    return result;
  } catch (error) {
    const errorMessage = error.message || '获取 ' + source + ' 封面时发生未知错误';
    console.error('[' + pluginInfo.name + '] Error: ' + errorMessage);
    throw new Error(errorMessage);
  }
}
