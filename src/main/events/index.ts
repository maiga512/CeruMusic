...
function basisEvent(mainWindow: BrowserWindow) {
  ...
  mainWindow.on('close', () => {
    lyricWindow.getWin()?.close()
    // 检查是否应该真正退出应用
    const closeToTray = configManager.get('closeToTray', true)
    if (!closeToTray) {
      // 强制退出，不留后台
      app.exit(0)
    }
  ...
  ipcMain.on('settings:sync-close-to-tray', (_, value: boolean) => {
    configManager.set('closeToTray', value)
    console.log(`[settings] closeToTray synced to main process: ${value}`)
  })

  // 获取当前 closeToTray 设置
  ipcMain.handle('settings:get-close-to-tray', () => {
    return configManager.get('closeToTray', true)
  })
}
