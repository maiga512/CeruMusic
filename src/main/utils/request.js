...
// Agent 缓存，防止每次请求新建 HttpsProxyAgent/HttpProxyAgent 导致 socket 泄露
...
const getRequestAgent = (url) => {
  ...
  if (proxyUrl) {
    const cacheKey = `${httpsRxp.test(url) ? 'https' : 'http'}:${proxyUrl}`
    if (!agentCache.has(cacheKey)) {
      const Agent = httpsRxp.test(url) ? HttpsProxyAgent : HttpProxyAgent
      agentCache.set(cacheKey, new Agent({ proxy: proxyUrl }))
    }
