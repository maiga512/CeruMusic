...
onMounted(async () => {
  console.log('[welcome] onMounted started')
  ...
  try {
    ...
    console.log('[welcome] get-app-version result:', appVersion)
    if (appVersion) version.value = appVersion
  } catch (error) {
    console.warn('Failed to get app version:', error)
  }
  console.log('[welcome] version resolved, continuing init')
  ...
  // 模拟加载步骤提示
  setTimeout(() => (loadingText.value = '加载界面资源...'), 500)

  loadingPercent.value = 85
  loadingText.value = '正在进入主页...'
  ...
  router.replace('/home').then(() => {
    setTimeout(() => {
      initPlayback().catch((e) => console.error('initPlayback failed', e))
