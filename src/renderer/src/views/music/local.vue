...
async function coverLoader(song: MusicItem, signal: AbortSignal) {
  ...
  const syncCurrentSongCover = (cover: string) => {
    const current = currentPlayingSongInfo.value
    if (
      cover &&
      current &&
      String(current.songmid) === String(song.songmid) &&
  ...
  if (song?.hasCover && api?.localMusic?.getCoverBase64) {
    const data = await api.localMusic.getCoverBase64(String(song.songmid))
    if (signal.aborted) return ''
    if (data) {
      song.hasCover = true
      syncCurrentSongCover(data)
      return data
    }
    song.hasCover = false
  }
