...
// 封面色提取结果缓存，避免同一张封面反复解码+K-means
const colorCache = new Map<string, { dominantColor: Color; useBlackText: boolean }>()
const CACHE_MAX = 50
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        colorCache.set(imageSrc, { dominantColor, useBlackText })
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        colorCache.set(imageSrc, { dominantColor: enhancedColor, useBlackText: false })
