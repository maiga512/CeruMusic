...
// settings storeToRefs currently unused but kept for future settings access
const {} = storeToRefs(settingsStore)
...
const getSongFromListById = (songId: number | string | null | undefined) => {
  if (songId === null || songId === undefined) return null
  return list.value.find((song) => song.songmid === songId) || null
}

const getRestoredContextSong = () => {
  return getSongFromListById(userInfo.value.lastPlaySongId) ||
...
// 加载本地歌曲内嵌封面（从文件读取 base64）
const loadLocalEmbeddedCover = async (song: SongList | Record<string, any>) => {
  const trackId = String((song as any).songmid)
  if (!trackId) return
  try {
    const api = (window as any).api
    if (!api?.localMusic?.getCoverBase64) return
    const base64 = await api.localMusic.getCoverBase64(trackId)
    if (base64 && typeof base64 === 'string') {
      songInfo.value.img = base64
    }
  } catch {
    // 静默失败，不影响播放
  }
...
const handlePlay = async () => {
  ...
  if (!Audio.value.url) {
    const restoredSong = getRestoredContextSong()
    if (restoredSong) {
      await playSong(restoredSong)
...
const playSong = async (song: SongList) => {
  ...
  try {
    ...
    globalPlayStatus.updatePlayerInfo({
      ...song,
    ...
    if (
      song.source === 'local' &&
      Boolean((song as any).hasCover) &&
      !normalizeCoverUrl(song.img)
    ) {
      void loadLocalEmbeddedCover(song)
    }
...
const normalizePlayMode = (mode: unknown): PlayMode => {
  if (mode === PlayMode.RANDOM) return PlayMode.RANDOM
  if (mode === PlayMode.SINGLE) return PlayMode.SINGLE
...
const ensureShuffleOrder = (rebuild: boolean = false) => {
...
const syncPlayModeFromUserInfo = (mode: unknown = userInfo.value.playMode) => {
  const normalized = normalizePlayMode(mode)
  if (playMode.value !== normalized) {
    playMode.value = normalized
  }
  if (userInfo.value.playMode !== normalized) {
    userInfo.value.playMode = normalized
  }
}

watch(
  () => userInfo.value.playMode,
  (mode) => {
    syncPlayModeFromUserInfo(mode)
  },
  { immediate: true }
)
...
const resolveNextSong = ({
  ...
  if (playMode.value === PlayMode.RANDOM) {
    ...
    return getSongFromListById(nextId)
  }
...
const initPlayback = async () => {
  ...
  if (userInfo.value.lastPlaySongId && list.value.length > 0) {
    ...
    if (lastPlayedSong) {
      ...
      if (!normalizeCoverUrl(lastPlayedSong.img)) {
      }
      if (!Audio.value.isPlay) {
        try {
          ...
          mediaSessionController.updateMetadata({
            ...
            artworkUrl: lastPlayedSong.img || ''
          })
        } catch (error) {
          console.warn('恢复上次播放歌曲 URL 失败，等待点击播放时补建上下文', error)
        }
