...
export const useGlobalPlayStatusStore = defineStore(
  ...
  () => {
          ...
          ;(window as any).api.localMusic
            ...
            .then((data: string) => {
              if (
                ...
                const queueSong = localUserStore.list.find(
                  (song: any) => String(song.songmid) === targetId && song.source === 'local'
                )
                if (queueSong && queueSong.img !== data) {
                  queueSong.img = data
                }
              }
            })
    ...
    function updatePlayerInfo(songInfo: Partial<SongList> & { songmid: null | number | string }) {
      if (player.songInfo?.songmid === songInfo.songmid) {
        player.songInfo = {
          ...(player.songInfo || DEFAULT_SONG_INFO),
          ...songInfo
        } as SongList
        return
      }
      player.songInfo = songInfo as SongList
    }
