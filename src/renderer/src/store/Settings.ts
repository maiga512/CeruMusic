...
export interface SettingsState {
  ...
  // 缓存管理设置
  maxCacheSizeGB?: number // 最大缓存大小（GB）
  cacheCleanupThreshold?: number // 清理阈值（0.5-1.0）
...
export const useSettingsStore = defineStore(
  ...
  () => {
      ...
      try {
        ...
        if (saved) {
          ...
          return {
            ...
            // 缓存管理设置
            maxCacheSizeGB: parsed.maxCacheSizeGB ?? defaultSettings.maxCacheSizeGB,
            cacheCleanupThreshold: parsed.cacheCleanupThreshold ?? defaultSettings.cacheCleanupThreshold,
            autoCleanupEnabled: parsed.autoCleanupEnabled ?? defaultSettings.autoCleanupEnabled
          }
        }
      } catch (error) {
    ...
    const saveSettings = () => {
      ...
      // 缓存管理设置默认值
      if (typeof settings.value.maxCacheSizeGB === 'undefined') {
        settings.value.maxCacheSizeGB = 5
      }
      if (typeof settings.value.cacheCleanupThreshold === 'undefined') {
        settings.value.cacheCleanupThreshold = 0.9
      }
      if (typeof settings.value.autoCleanupEnabled === 'undefined') {
        settings.value.autoCleanupEnabled = true
      }
      localStorage.setItem('appSettings', JSON.stringify(settings.value))

      // 同步 closeToTray 设置到主进程
      if (typeof settings.value.closeToTray !== 'undefined') {
        ;(window.api as any)?.settings?.syncCloseToTray(settings.value.closeToTray)
      }
    }
