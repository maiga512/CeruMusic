...
async function init() {
  ...
  const audioEl = document.getElementById('globalAudio') as HTMLAudioElement | null
...
async function start() {
  ...
  // Request microphone permission first (for macOS)
  try {
...
<template>
  ...
  >
    ...
    <TransitionGroup name="fade-group" tag="div" class="label-group">
      ...
      <span v-if="isHovering" key="desc" class="match-desc">识别电脑正在播放的声音或上传音频文件</span>
    </TransitionGroup>
    ...
    <Transition name="fade">
      <div v-if="micPermissionDenied" class="permission-tip">
        请在系统设置中开启麦克风权限
      </div>
    </Transition>
  </div>
